from .preprocessing import calculate_jitter, calculate_jitter_dispersion
from .signal_energy import JitterDispersion
from .kstest import KSTest

def compute_jitter_dispersion(change_points, epoch_jitter, jitter, K, L, jitter_dispersion_threshold=0.25):
    """
    ...
    epoch_jitter_dispersion, jitter_dispersion = calculate_jitter_dispersion(epoch_jitter, jitter, K, L)
...
def compute_ks_test(change_points, epoch_rtt, rtt):
    ...
    epoch_jitter, jit = calculate_jitter(epoch_rtt, rtt)

    # Perform KS test
    ks = KSTest(epoch_jitter, jit, change_points)
    ks.fit()
